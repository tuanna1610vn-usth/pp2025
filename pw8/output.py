...
def output(markManagement, stdscr):
    curses.curs_set(1)
    markManagement.display(stdscr)
    markManagement.rankings(stdscr)

    stdscr.clear()
    stdscr.addstr(10, 15, "Thank you for using Student Management System!", curses.A_BOLD)
...
def saveZip(markManagement, stdscr):
    try:
        # Write data to text file first
        markManagement.saveFiles()
